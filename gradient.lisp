...
(defun gradient-horiz (w h x1 x2 c1 c2 dither) 
  ...
  (let ((data (make-array (list w h))))
    (dotimes (j h)
      (let ((val (calc-col (abs (- j x1)) (abs (- j x2)) c1 c2 dither)))
	(dotimes (i w)
...
(defun gradient-verti (w h y1 y2 c1 c2 dither)
  ...
  (let ((data (make-array (list w h))))
    (dotimes (i w)
      (let ((val (calc-col (abs (- i y1)) (abs (- i y2)) c1 c2 dither)))
	(dotimes (j h)
	  (setf (aref data i j) val))))
    data))
...
(defun gradient (w h x1 y1 x2 y2 c1 c2 dither) 
  (if (and (= x1 x2)(= y1 y2)) (error "Gradient points must be different"))
  (cond
    ((= x1 x2) (gradient-verti w h y1 y2 c1 c2 dither))
    ((= y1 y2) (gradient-horiz w h x1 x2 c1 c2 dither))
