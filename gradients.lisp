...
(defun make-multistop-table (stops &key (steps nil) (interpolation :linear))
  ...
  (let ((steps (if steps steps (* *steps-per-stop* (length stops))))
    ...
    (let ((color-table (make-array steps :element-type 'list))
	  (prev-stop (car stops))
	  (next-stop (car stops))
	  (remaining-stops (cdr stops)))
      (flet ((interpol (c1 c2 x)
	       (mapcar #'round
		       (ecase interpolation
			 (:linear (mapcar #'(lambda (a b)
					      (+ (* a (- 1 x)) (* b x)))
					  c1 c2))
			 (:sin2 (mapcar #'(lambda (a b)
					    (+ (* a (expt (sin (/ (* pi (- 1 x)) 2)) 2))
					       (* b (expt (sin (/ (* pi x) 2)) 2))))
					c1 c2))))))
       (dotimes (k steps color-table)
	 (let ((x (/ k (1- steps))))
	  (loop while (> x (car next-stop)) do
	    (setf prev-stop next-stop
		  next-stop (if remaining-stops
				(car remaining-stops)
				next-stop)
		  remaining-stops (cdr remaining-stops)))
	  (if (eql (car prev-stop) (car next-stop))
	      (setf (aref color-table k) (cdr prev-stop))
	      (let ((x-stops (/ (- x (car prev-stop))
				(- (car next-stop) (car prev-stop)))))
		(setf (aref color-table k)
		      (interpol (cdr prev-stop)
				(cdr next-stop)
				x-stops))))))))))